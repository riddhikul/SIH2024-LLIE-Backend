...
def dhe(img, alpha=0.5):
    ...
    hsv = matplotlib.colors.rgb_to_hsv(np.asarray(img))
    h = hsv[:, :, 0]
    s = hsv[:, :, 1]
...
async def upload_single_image(file: UploadFile = File(...)):
    ...
    output_path = os.path.join("app/static/output", output_filename)
    imageio.imwrite(output_path, result)

    return {"filename": file.filename, "processed_image": output_filename, "message": "File successfully uploaded and processed"}

@router.get("/download/{filename}")
async def download_image(filename: str):
    # Define the path where the processed files are stored
